...
describe('APP/TASK', ()=>{
    ...
    it('POST',async()=>{
        ...
        expect(task.post([ {...itm, StartTime: addTime(itm.StartTime, -1)} ])).rejects.toThrow(MESSAGES.TASK.RULES.POST.MINIMAL_START);
        expect(task.post([ {...itm, EndTime: addTime(itm.StartTime, -1)} ])).rejects.toThrow(MESSAGES.TASK.RULES.POST.START_END);        
    ...
    it('DELETE', async()=>{
        ...
        await expect(task.list({IdOwner, Scene: 'MONTH', DateRef: new Date()})).resolves.toHaveLength(0);
        await expect(task.list({IdOwner, Scene: 'WEEK', DateRef: new Date()})).resolves.toHaveLength(0);
        await expect(task.list({IdOwner, Scene: 'DAY', DateRef: new Date()})).resolves.toHaveLength(0);
    })
    ...
    it('LIST', async()=>{
        ...
        await expect(task.list({IdOwner: owner, Scene: 'MONTH', DateRef: new Date(_y, _m, _d)})).resolves.toHaveLength( 6 );
        await expect(task.list({IdOwner: owner, Scene: 'WEEK',  DateRef: new Date(_y, _m, _d)})).resolves.toHaveLength( 3 );
        await expect(task.list({IdOwner: owner, Scene: 'DAY',   DateRef: new Date(_y, _m, _d)})).resolves.toHaveLength( 2 );
        
        await expect(task.list({IdOwner: owner, Scene: 'MONTH', DateRef: new Date(_y, _m+1, _d)})).resolves.toHaveLength( 6 );
        await expect(task.list({IdOwner: owner, Scene: 'WEEK',  DateRef: new Date(_y, _m+1, _d)})).resolves.toHaveLength( 2 );
        await expect(task.list({IdOwner: owner, Scene: 'DAY',   DateRef: new Date(_y, _m+1, _d)})).resolves.toHaveLength( 2 );
